  ...
  class TreeViewSWT
    ...
    class TreeMirrorContentProvider
      ...
      def get_elements(tree_mirror)
        sort_elements(tree_mirror.top).to_java
      end
      ...
      def get_children(tree_node)
        sort_elements(tree_node.children).to_java
      end
      ...
      def sort_elements(elements)
        ...
        elements.sort_by do |c|
          c.path.split(File::SEPARATOR).last
        end.sort_by do |c|
          File.directory?(c.path) ? -1 : 1
        end
      end
    end
