...
P2END
  ...
  def test_re_parse_line_with_extra_opening_scopes
    ...
    assert_equal 11, smp.scope_tree.children.length # <- this is not up to date for the entire text.
  end
...
APE
  ...
  def test_shift_after
    ...
    rubycode="class Redcar::File\n  def nice_name\n    @filename.split(\"/\").last\n  end\nend\n"
    smp.add_lines(rubycode)
    assert_equal [0, 1, 2, 2, 2, 2, 2, 2, 3, 4], smp.scope_tree.children.map{|c| c.start.line}
    smp.shift_after(2, 2)
    assert_equal [0, 1, 4, 4, 4, 4, 4, 4, 5, 6], smp.scope_tree.children.map{|c| c.start.line}
  end
