...
module GrammarTest
  ...
  def test_load_basic_properties
    ...
    assert_equal ORegexp.new("#! /usr/bin/ruby"), gr.first_line_match
  ...
  def test_load_pattern_from_hash
    ...
    assert_equal ORegexp.new(hash['begin']), pat.begin
  ...
  def test_load_pattern_from_hash_captures
    ...
    pat = gr.pattern_from_hash(hash)
    assert_equal ORegexp.new("(@selector\\()(.*?)(\\))"), pat.match
  ...
  def test_load_loads_patterns
    ...
    assert_equal ORegexp.new("\\b(if|while|for|return)\\b"), gr.patterns[0].match
    assert_equal "string.quoted.double.untitled", gr.patterns[1].name.to_s
    assert_equal ORegexp.new('"'), gr.patterns[1].begin
  ...
  def test_load_repository
    ...
    assert_equal ORegexp.new("\\("), gr.repository.values[0].begin
  end
  ...
  def test_pattern_lookup
    gr = Grammar.new(@grammar1)
    assert_equal ORegexp.new("\\b(if|while|for|return)\\b"), gr.pattern('keyword.control.untitled').match
    assert_equal ORegexp.new('"'), gr.pattern('string.quoted.double.untitled').begin
    assert_equal ORegexp.new("\\."), gr.pattern('constant.character.escape.untitled').match
  end
  ...
  def test_possible_patterns_with_embedded_grammar
    SyntaxSourceView.load_grammars
