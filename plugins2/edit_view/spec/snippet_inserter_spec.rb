...
describe Redcar::EditView::SnippetInserter do
  ...
  before(:each) do
    ...
    @snippet_inserter = Redcar::EditView::SnippetInserter.new(@buf)
    @ap = Redcar::EditView::AutoPairer.new(@buf)
  end
  ...
  def press_tab(si=@snippet_inserter, buf=@buf)
    Redcar::TabPressed.new(si, buf).do
  end
